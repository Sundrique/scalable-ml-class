...
def parsePoint(line):
    ...
    """
    splitted = line.split(',')
    return LabeledPoint(splitted[0], splitted[1:])
...
def calcRMSE(labelsAndPreds):
    ...
    return ((labelsAndPreds
            ...
            .reduce(lambda a, b: a + b)) * 1.0 / labelsAndPreds.count()) ** 0.5
...
def linregGradientDescent(trainData, numIters):
    ...
    for i in range(numIters):
        ...
        gradient = (trainData
                    .map(lambda lp: gradientSummand(w, lp))
...
# TODO: Replace <FILL IN> with appropriate code
firstModel = LinearRegressionWithSGD.train(parsedTrainData, numIters, step=1.0, miniBatchFraction=1.0, initialWeights=None, regParam=1e-1, regType='l2', intercept=True)
...
def twoWayInteractions(lp):
    ...
    """
    return LabeledPoint(lp.label, list(lp.features) + [x[0] * x[1] for x in itertools.product(lp.features, repeat=2)])
